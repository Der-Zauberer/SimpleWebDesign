class Swd{#loaded=!1
    ...
    this.#observer=new MutationObserver((mutationList,observer)=>{this.swdOnUpdate([...mutationList])})
    this.#observer.observe(this,{attributes:!0,childList:!0,subtree:!1})
    ...
    #selfUpdateQueue=0
    swdOnUpdate(){if(this.#selfUpdateQueue-->0)return
    const input=this.querySelector('input')
    if(input!==this.input){if(this.#input)this.#input.removeEventListener('input',event=>this.#updateValidation())
    ...
    this.#selfUpdateQueue++}}}
    class SwdDropdown extends SwdComponent{static #openDropdowns=[]
    #dropdownInput;#dropdownSecondaryInput;#dropdownContent;#selection;#FOCUS_EVENT=event=>{this.open()}
    #BLUR_EVENT=event=>{if(this.#selection)this.#selection.select(event.explicitOriginalTarget);this.close()}
    swdOnInit(){this.swdRegisterManagedEvent(this,'click',event=>{if(!this.#dropdownContent)return
    if(!this.#dropdownInput){if(!this.isOpen())this.open()
    else this.close()}
    if(this.#selection)this.#selection.select(event.target)})
    this.swdRegisterManagedEvent(this,'keydown',event=>{if(this.#selection&&this.#dropdownInput&&!this.isOpen()&&event.key==='Enter'){event.preventDefault()
    this.open()
    return}
    if(!this.#selection||!this.isOpen())return
    switch(event.key){case 'ArrowUp':case 'ArrowLeft':this.#selection.previous()
    event.preventDefault()
    break
    case 'ArrowDown':case 'ArrowRight':this.#selection.next()
    event.preventDefault()
    break
    ...
    break
    case 'Delete':case 'Backspace':if(this.#dropdownInput&&this.#dropdownInput.hasAttribute('readonly'))this.#dropdownInput.value=''
    break}})}
    swdOnUpdate(event){if(this.#dropdownInput){this.#dropdownInput.removeEventListener('focus',this.#FOCUS_EVENT)
    this.#dropdownInput.removeEventListener('blur',this.#BLUR_EVENT)}
    this.#dropdownContent=this.querySelector('swd-dropdown-content');this.#dropdownInput=this.querySelector('swd-dropdown input:not(swd-dropdown-content *)')
    if(this.#dropdownInput){this.#dropdownInput.addEventListener('focus',this.#FOCUS_EVENT)
    ...
    if(this.#selection){this.#selection.setOnSelect((text,value)=>{if(this.#dropdownInput)this.#dropdownInput.value=value})}}
    open(){if(!this.#dropdownContent)return
    ...
    if(contentRect.bottom>window.innerHeight){if(dropdownRect.top-contentRect.height>window.innerHeight-contentRect.bottom){if(dropdownRect.top-contentRect.height<0){this.#dropdownContent.style.maxHeight=`${dropdownRect.top}px`}
    ...
    static closeAllDropdowns(){for(const dropdown of SwdDropdown.#openDropdowns)dropdown.close()}}
    class SwdSelection extends SwdComponent{#selectionChangeAction
    value
    swdOnInit(){this.swdRegisterManagedEvent(this,'click',event=>{this.select(event.target)})}
    next(){this.#nextOrPrevious(!0)}
    previous(){this.#nextOrPrevious(!1)}
    #nextOrPrevious(next){const selected=this.querySelector('[selected]')
    ...
    this.select(target)}
    select(target){if(!target||target.nodeName!=='A')return
